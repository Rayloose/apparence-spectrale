...
def calcul_brdf(albedo, theta, n):
    """
    Retourne la brdf à partir d'un albédo

    Args:
        albedo (liste de float): Une liste contenant les valeurs de l'albédo d'un spectre entre 380nm et 780nm.
        theta (float): angle entre la direction source et la direction d'observation'
    Returns:
    ...
    kd = albedo
    ks = np.array([1 for i in range (len(albedo))])
    
    brdf = kd + ks*(n+1)/(n+2)*np.cos(theta)**n #Modèle de Phong
...
for i in range(1, 9):
    ...
    brdf = calcul_brdf(albedo_np, theta, n)   
